...
class FloatingText:
    """Texto que sobe e desaparece (+500)"""
    def __init__(self, x, y, text, color):
        ...
        self.color = color
        self.timer = 0
        self.max_time = 60 # frames (1 segundo)
...
class Particle:
    """Partículas multi-uso (Rastro e Explosão)"""
    def __init__(self, x, y, color=WHITE, explosive=False):
        ...
        if explosive:
            # Explode em todas as direções
            speed = random.randint(100, 300)
            angle = random.uniform(0, 6.28)
            self.vx = math.cos(angle) * speed
            self.vy = math.sin(angle) * speed
            self.decay = random.randint(5, 10) # Dura mais
        else:
            # Rastro para trás
...
class Cloud:
    ...
    def draw(self, surface):
        s = pygame.Surface((int(100*self.scale), int(60*self.scale)), pygame.SRCALPHA)
        color = (255, 255, 255, 180) 
        pygame.draw.circle(s, color, (int(30 * self.scale), int(30 * self.scale)), int(25 * self.scale))
        pygame.draw.circle(s, color, (int(50 * self.scale), int(20 * self.scale)), int(35 * self.scale))
        pygame.draw.circle(s, color, (int(70 * self.scale), int(30 * self.scale)), int(25 * self.scale))
...
class Capivara:
    ...
    def draw(self, surface):
        if self.immunity_timer > 0:
            if int(self.immunity_timer * 15) % 2 == 0:
                return
        ...
        try:
            pygame.draw.rect(surface, BROWN_DARK, focinho_rect, border_top_right_radius=5, border_bottom_right_radius=5)
        except TypeError:
            pygame.draw.rect(surface, BROWN_DARK, focinho_rect)
...
class Pipe:
    ...
    def update(self, dt, speed):
        ...
        for c in self.collectibles:
            if not c.collected:
                c.update_position(cx, self.gap_mid)
        for p in self.powerups:
            if not p.collected:
                p.update(dt, speed, self.gap_mid)
...
class Game:
    ...
    def reset(self):
        ...
        self.pipes = []
        self.particles = []
        self.floating_texts = [] # Lista de textos flutuantes
        self.clouds = [Cloud(random.randint(0, WIDTH)) for _ in range(6)]
        ...
        self.shake_duration = 0 # Screen shake
        self.level_up_flash = 0 # Flash visual
    ...
    def update(self, dt):
        ...
        if self.game_over:
            if not self.saved_this_session:
                ...
                self.shake_duration = 10 # Shake ao morrer
        ...
        if self.current_speed < MAX_SPEED:
            self.current_speed += 4 * dt

        new_multiplier = 1 + int(self.time // 10)
        if new_multiplier > self.multiplier:
            self.multiplier = new_multiplier
            self.level_up_flash = 255 # Flash!

        self.capy.update(dt)
        self.ground.update(dt, self.current_speed)
        
        if random.random() < 0.3:
            self.particles.append(Particle(self.capy.x - 10, self.capy.y))
            ...
            if pipe.collides_with(self.capy.get_rect()):
                if self.capy.immunity_timer <= 0:
                    if self.capy.has_shield:
                        ...
                        self.capy.y -= 10
                        self.shake_duration = 10 # Shake impacto
                        # Particulas de escudo quebrando
    ...
    def draw_start(self):
        ...
        # Capivara flutuando no menu (já desenhada pelo update)
    ...
    def draw_game(self):
        ...
        # Para fazer o shake, precisaríamos de uma surface intermediária
        # Como é complexo mudar agora, vamos aplicar shake só nos objetos principais visualmente ou ignorar
        # Vamos manter simples: Shake só afeta o desenho se redesenharmos tudo com offset
        # Simplificação: Não aplicamos offset global para não quebrar a lógica de UI fixa
